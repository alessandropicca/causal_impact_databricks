...
def get_target_metric_in_perim_query():
    ...
    within single graph bracket embedded in single quotes.
    ...
    """
    input_query = """select A.reference_date, A.subject AS campaign_dimension ,sum(A.gmv_proceeds_usd) AS metric
                        from(
                        select * , CAST(payment_ts as DATE) as reference_date
                        from default.fact_payment
                        where CAST(payment_ts as DATE) between '{start}' and '{end}'
                        and subject = '{target}'
                        )A
                        group by A.reference_date, A.subject
                        order by A.subject, A.reference_date
                    """
    return input_query
...
def get_target_metric_control_query():
    ...
    The query accepts three parameters, the first two are named '{start}', '{end}' and need to be expressed
    within single graph bracket embedded in single quotes.The third paramether is named {control} and MUST be expressed within single graph brakcet WITHOUT 
    any single quote incapsulation.
    The first one states the starting date of the observation period while the second is the ending date.
...
def get_npc_control_query():
    ...
    input_query = """select date as reference_date, country_code as campaign_dimension, sum(npc_first_touch) as npc
                        from default.ci_anl_table
                        where date between '{start}' and '{end}'
                        and traffic_group != 'CONTENT'
                        and subject in {control}
